...
is
   ...
   type Virtual_Field is (F_Initial, F_Destination, F_Source, F_Type_Length_TPID, F_TPID, F_TCI, F_Type_Length, F_Ether_Type, F_Payload, F_Final);
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First; Frame_Size : Ethernet.Type_Length := Ethernet.Type_Length'First) is private with
     Default_Initial_Condition =>
   ...
   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; Frame_Size : Ethernet.Type_Length; Written_Last : RFLX_Types.Bit_Length := 0) with
     Pre =>
       not Ctx'Constrained
     ...
     Post =>
       ...
       and Ctx.Frame_Size = Frame_Size
       and Initialized (Ctx),
     Depends =>
       (Ctx => (Buffer, Frame_Size, Written_Last), Buffer => null);

   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Frame_Size : Ethernet.Type_Length; Written_Last : RFLX_Types.Bit_Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then Buffer /= null
       and then Buffer'Length > 0
     ...
     Post =>
       ...
       and Initialized (Ctx),
     Depends =>
       (Ctx => (Buffer, First, Last, Frame_Size, Written_Last), Buffer => null);
   ...
   procedure Reset (Ctx : in out Context; Frame_Size : Ethernet.Type_Length) with
     Pre =>
       not Ctx'Constrained
   ...
   procedure Reset (Ctx : in out Context; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Frame_Size : Ethernet.Type_Length) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Ethernet.Frame.Has_Buffer (Ctx)
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX_Types.Bytes_Ptr) with
     ...
     Post =>
       ...
       and Ctx.Frame_Size = Ctx.Frame_Size'Old
   ...
   function Get_Ether_Type (Ctx : Context) return RFLX.Ethernet.Ether_Type with
     Pre =>
       RFLX.Ethernet.Frame.Valid (Ctx, RFLX.Ethernet.Frame.F_Ether_Type);
   ...
   procedure Set_Type_Length_TPID (Ctx : in out Context; Val : RFLX.Ethernet.Type_Length) with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Payload)
       and (if
               Get_Type_Length_TPID (Ctx) >= 1536
               and Get_Type_Length_TPID (Ctx) /= 16#8100#
            then
               Predecessor (Ctx, F_Ether_Type) = F_Type_Length_TPID
               and Valid_Next (Ctx, F_Ether_Type))
       and (if
               Get_Type_Length_TPID (Ctx) <= 1500
            then
               Predecessor (Ctx, F_Payload) = F_Type_Length_TPID
   ...
   procedure Set_Type_Length (Ctx : in out Context; Val : RFLX.Ethernet.Type_Length) with
     ...
     Post =>
       ...
       and (if
            ...
            then
               Predecessor (Ctx, F_Ether_Type) = F_Type_Length
               and Valid_Next (Ctx, F_Ether_Type))
       and (if
       ...
       and Get_Source (Ctx) = Get_Source (Ctx)'Old
       and Get_Type_Length_TPID (Ctx) = Get_Type_Length_TPID (Ctx)'Old
       and Get_TCI (Ctx) = Get_TCI (Ctx)'Old
       and Field_First (Ctx, F_Type_Length) = Field_First (Ctx, F_Type_Length)'Old
       and Field_Last (Ctx, F_Type_Length) = Field_Last (Ctx, Predecessor (Ctx, F_Type_Length)) + Field_Size (Ctx, F_Type_Length)
       and (for all F in Field range F_Destination .. F_TCI =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));

   procedure Set_Ether_Type (Ctx : in out Context; Val : RFLX.Ethernet.Ether_Type) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Ethernet.Frame.Has_Buffer (Ctx)
       and then RFLX.Ethernet.Frame.Valid_Next (Ctx, RFLX.Ethernet.Frame.F_Ether_Type)
       and then RFLX.Ethernet.Valid_Ether_Type (RFLX.Ethernet.To_Base_Integer (Val))
       and then RFLX.Ethernet.Frame.Available_Space (Ctx, RFLX.Ethernet.Frame.F_Ether_Type) >= RFLX.Ethernet.Frame.Field_Size (Ctx, RFLX.Ethernet.Frame.F_Ether_Type)
       and then RFLX.Ethernet.Frame.Field_Condition (Ctx, RFLX.Ethernet.Frame.F_Ether_Type, RFLX.Ethernet.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Ether_Type)
       and Get_Ether_Type (Ctx) = Val
       and Invalid (Ctx, F_Payload)
       and (Predecessor (Ctx, F_Payload) = F_Ether_Type
            and Valid_Next (Ctx, F_Payload))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Ctx.Frame_Size = Ctx.Frame_Size'Old
       and Predecessor (Ctx, F_Ether_Type) = Predecessor (Ctx, F_Ether_Type)'Old
       and Valid_Next (Ctx, F_Ether_Type) = Valid_Next (Ctx, F_Ether_Type)'Old
       and Get_Destination (Ctx) = Get_Destination (Ctx)'Old
       and Get_Source (Ctx) = Get_Source (Ctx)'Old
       and Get_Type_Length_TPID (Ctx) = Get_Type_Length_TPID (Ctx)'Old
       and Field_First (Ctx, F_Ether_Type) = Field_First (Ctx, F_Ether_Type)'Old
       and (for all F in Field range F_Destination .. F_Type_Length =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));
...
private
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors; Frame_Size : Ethernet.Type_Length) return Boolean is
     ((if Buffer /= null then Buffer'First = Buffer_First and Buffer'Last = Buffer_Last)
                ...
                and then (if
                          ...
                          then
                             (Valid (Cursors (F_Type_Length))
                              and then Cursors (F_Ether_Type).Predecessor = F_Type_Length
                              and then Cursors (F_Type_Length).Value >= 1536)
                             or (Valid (Cursors (F_Type_Length_TPID))
                                 and then Cursors (F_Ether_Type).Predecessor = F_Type_Length_TPID
                ...
                and then (if
                          ...
                          then
                             (Valid (Cursors (F_Ether_Type))
                              and then Cursors (F_Payload).Predecessor = F_Ether_Type)
                             ...
                             or (Valid (Cursors (F_Type_Length_TPID))
                                 and then Cursors (F_Payload).Predecessor = F_Type_Length_TPID
      ...
      and then ((if Invalid (Cursors (F_Destination)) then Invalid (Cursors (F_Source)))
                ...
                and then (if Invalid (Cursors (F_TCI)) then Invalid (Cursors (F_Type_Length)))
                and then (if
                             Invalid (Cursors (F_Type_Length))
                             and then Invalid (Cursors (F_Type_Length_TPID))
                          then
                             Invalid (Cursors (F_Ether_Type)))
                and then (if
                             Invalid (Cursors (F_Ether_Type))
                             and then Invalid (Cursors (F_Type_Length))
                             and then Invalid (Cursors (F_Type_Length_TPID))
                          then
                                          ...
                                          then
                                             ...
                                             and then (if
                                                          ...
                                                          and then (Cursors (F_Type_Length_TPID).Value >= 1536
                                                                    and Cursors (F_Type_Length_TPID).Value /= 16#8100#)
                                                       then
                                                          Cursors (F_Ether_Type).Last - Cursors (F_Ether_Type).First + 1 = 16
                                                          and then Cursors (F_Ether_Type).Predecessor = F_Type_Length_TPID
                                                          and then Cursors (F_Ether_Type).First = RFLX_Types.Bit_Index (Cursors (F_Type_Length_TPID).First)
                                                          and then (if
                                                                       Well_Formed (Cursors (F_Payload))
                                                                    then
                                                                       Cursors (F_Payload).Last - Cursors (F_Payload).First + 1 = RFLX_Types.Bit_Length (Frame_Size) * 8 + ((-RFLX_Types.Bit_Length (Cursors (F_Ether_Type).Last)) + RFLX_Types.Bit_Length (First) - 1)
                                                                       and then Cursors (F_Payload).Predecessor = F_Ether_Type
                                             ...
                                             and then (if
                                                          ...
                                                          and then Cursors (F_Type_Length_TPID).Value <= 1500
                                                       then
                                                          Cursors (F_Payload).Last - Cursors (F_Payload).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Type_Length_TPID).Value) * 8
                                                          and then Cursors (F_Payload).Predecessor = F_Type_Length_TPID
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First; Frame_Size : Ethernet.Type_Length := Ethernet.Type_Length'First) is
      record
     ...
     Dynamic_Predicate =>
       Valid_Context (Context.Buffer_First, Context.Buffer_Last, Context.First, Context.Last, Context.Verified_Last, Context.Written_Last, Context.Buffer, Context.Cursors, Context.Frame_Size);
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     ((case Fld is
          ...
          when F_Type_Length =>
             RFLX.Ethernet.Valid_Type_Length (Val),
          when F_Ether_Type =>
             RFLX.Ethernet.Valid_Ether_Type (Val),
          when F_Payload =>
             True));
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length is
     ((case Fld is
          ...
          when F_Payload =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Ether_Type
              then
                 RFLX_Types.Bit_Length (Ctx.Frame_Size) * 8 + ((-RFLX_Types.Bit_Length (Ctx.Cursors (F_Ether_Type).Last)) + RFLX_Types.Bit_Length (Ctx.First) - 1)
              elsif
              ...
              then
                 ...
                 and then Ctx.Cursors (F_Type_Length_TPID).Value <= 1500
              then
                 RFLX_Types.Bit_Length (Ctx.Cursors (F_Type_Length_TPID).Value) * 8
              else
   ...
   function Get_Ether_Type (Ctx : Context) return RFLX.Ethernet.Ether_Type is
     (To_Actual (Ctx.Cursors (F_Ether_Type).Value));
