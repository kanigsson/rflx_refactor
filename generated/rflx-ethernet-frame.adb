...
is
   ...
   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; Frame_Size : Ethernet.Type_Length; Written_Last : RFLX_Types.Bit_Length := 0) is
   begin
      Initialize (Ctx, Buffer, RFLX_Types.To_First_Bit_Index (Buffer'First), RFLX_Types.To_Last_Bit_Index (Buffer'Last), Frame_Size, Written_Last);
   end Initialize;

   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Frame_Size : Ethernet.Type_Length; Written_Last : RFLX_Types.Bit_Length := 0) is
      Buffer_First : constant RFLX_Types.Index := Buffer'First;
      Buffer_Last : constant RFLX_Types.Index := Buffer'Last;
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, Frame_Size, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (F_Destination => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
      Buffer := null;
   end Initialize;

   procedure Reset (Ctx : in out Context; Frame_Size : Ethernet.Type_Length) is
   begin
      Reset (Ctx, RFLX_Types.To_First_Bit_Index (Ctx.Buffer'First), RFLX_Types.To_Last_Bit_Index (Ctx.Buffer'Last), Frame_Size);
   end Reset;

   procedure Reset (Ctx : in out Context; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Frame_Size : Ethernet.Type_Length) is
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, Frame_Size, First - 1, First - 1, Ctx.Buffer, (F_Destination => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
   end Reset;
   ...
   begin
      Reset (Ctx, RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First), RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last), Ctx.Frame_Size);
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          ...
          when F_Type_Length_TPID =>
             Invalid (Ctx.Cursors (F_Ether_Type))
             and Invalid (Ctx.Cursors (F_Payload))
             and Invalid (Ctx.Cursors (F_TPID)),
          when F_TPID =>
   ...
   begin
      for F in Field loop
         pragma Loop_Invariant (Has_Buffer (Ctx)
                                ...
                                and Ctx.Frame_Size = Ctx.Frame_Size'Loop_Entry);
         Verify (Ctx, F);
      end loop;
   end Verify_Message;
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     ...
     Post =>
       ...
       and then Ctx.Frame_Size = Ctx.Frame_Size'Old
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
                    ...
                    when F_Type_Length =>
                       ...
                       and (if
                            ...
                            then
                               ...
                               and Valid_Next (Ctx, F_Payload)),
                    when F_Ether_Type =>
                       Get_Ether_Type (Ctx) = To_Actual (Val)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Ethernet.Frame.Valid_Next (Ctx, Fld)
       and then Fld in F_Destination | F_Source | F_Type_Length_TPID | F_TPID | F_TCI | F_Type_Length | F_Ether_Type
       and then RFLX.Ethernet.Frame.Valid_Value (Fld, Val)
   ...
   procedure Set_Ether_Type (Ctx : in out Context; Val : RFLX.Ethernet.Ether_Type) is
   begin
      Set_Scalar (Ctx, F_Ether_Type, RFLX.Ethernet.To_Base_Integer (Val));
   end Set_Ether_Type;
